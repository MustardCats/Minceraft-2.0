		...
		for (int y = cur_generating_y; y < cur_generating_y + num_rows; y++) {
			for (int z = 0; z < c_width; z++) {
				if (chunk->at(x, y, z)->id == 0)
					continue;
				glm::ivec3 pos = glm::vec3(chunk->x * c_length + x, chunk->y * c_height + y, chunk->z * c_width + z);
				//std::cout << chunk->at(x, y, z)->id << "\n";
				std::array<std::array<glm::vec2, 2>, 6> block_uvs = BlockTypes::getBlockUVs(chunk->at(x, y, z)->id);
				// front
				if ((z > 0 && chunk->at(x, y, z - 1)->id == 0) || (z == 0 && adj_chunks[0] != nullptr && adj_chunks[0]->at(x, y, c_width - 1)->id == 0)) {
					float front[] = {
				...
				// back
				if ((z < (c_width - 1) && chunk->at(x, y, z + 1)->id == 0) || (z == (c_width - 1) && adj_chunks[1] != nullptr && adj_chunks[1]->at(x, y, 0)->id == 0)) {
					float back[] = {
				...
				// left
				if ((x > 0 && chunk->at(x - 1, y, z)->id == 0) || (x == 0 && adj_chunks[2] != nullptr && adj_chunks[2]->at(c_length - 1, y, z)->id == 0)) {
					float left[] = {
				...
				// right
				if ((x < (c_length - 1) && chunk->at(x + 1, y, z)->id == 0) || (x == (c_length - 1) && adj_chunks[3] != nullptr && adj_chunks[3]->at(0, y, z)->id == 0)) {
					float right[] = {
				...
				// top
				if ((y < (c_height - 1) && chunk->at(x, y + 1, z)->id == 0) || (y < (c_height - 1) && adj_chunks[4] != nullptr && adj_chunks[4]->at(x, 0, z)->id == 0)) {
					float top[] = {
				...
				// bottom
				if ((y > 0 && chunk->at(x, y - 1, z)->id == 0) || (y == 0 && adj_chunks[5] != nullptr && adj_chunks[5]->at(x, c_height - 1, z)->id == 0)) {
					float bottom[] = {
